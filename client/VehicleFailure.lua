...
local function IsNearMechanic()
	...
	for _, item in pairs(repairCfg.mechanics) do
		local distance = #(vector3(item.x, item.y, item.z) - vector3(pedLocation["x"], pedLocation["y"], pedLocation["z"]))
...
RegisterNetEvent('mx-fix:repair')
AddEventHandler('mx-fix:repair', function()
	if isPedDrivingAVehicle() then
		local ped = PlayerPedId()		
		vehicle = GetVehiclePedIsIn(ped, false)		
		local engineHealth  = GetVehicleEngineHealth(vehicle)
		notification("Kendaraan sudah dibenarkan")
