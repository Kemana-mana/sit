...
local function RaycastCamera()
	local flag = -1
	local _retval, _hit, endCoords = false, false, false
	if Config.Target == 'qtarget' then
		endCoords = exports.qtarget:raycast(flag)
	elseif Config.Target == 'qb-target' then
		endCoords = exports['qb-target']:RaycastCamera(flag)
	else
...
local function AddCircleZone(name, center, radius, heading, minZ, maxZ, useZ, options)
	-- ID: 4872
	if Config.Target == 'ox_target' then
...
local function AddBoxZone(name, center, heading, length, width, height, minZ, maxZ, useZ, options)
	if Config.Target == 'ox_target' then
			...
			for name, info in pairs(data.polys) do
				...
				elseif info.lay == nil and info.sit == nil then
					...
					if info.poly.type == "circle" then
						...
						AddCircleZone(name, center, radius, heading, minZ, maxZ, true, options)
					else
						AddBoxZone(name, center, heading, info.poly.length, info.poly.width, info.poly.height, minZ, maxZ, true, options)
					end
				end
			end
